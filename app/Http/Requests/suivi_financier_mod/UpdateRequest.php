...
class UpdateRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'trimestre'             => ['bail','required', 'integer', 'min:1', 'max:4', Rule::unique('suivi_financier_mods','trimestre')->ignore($this->suivi_financier_mod)->where("siteId", $this->siteId)->where("annee", $this->annee)->whereNull('deleted_at'), 'size:'.$this->max],
    ...
    protected function prepareForValidation(): void
    {
        ...
        if(is_string($this->suivi_financier_mod))
        {
            $this->suivi_financier_mod = SuiviFinancierMod::findByKey($this->suivi_financier_mod);
        }

        if($this->annee ==  $this->suivi_financier_mod->annee)
            $count = $this->suivi_financier_mod->trimestre;
        else
        {
