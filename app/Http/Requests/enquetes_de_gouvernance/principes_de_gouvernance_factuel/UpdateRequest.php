...
class UpdateRequest extends FormRequest
{
    ...
    {
        if(is_string($this->principe_de_gouvernance_factuel))
        {
            $this->principe_de_gouvernance_factuel = PrincipeDeGouvernanceFactuel::findByKey($this->principe_de_gouvernance_factuel);
        }

        return [
            'nom'  => ['sometimes','max:255', Rule::unique('principes_de_gouvernance_factuel', 'nom')->where("programmeId", auth()->user()->programmeId)->ignore($this->principe_de_gouvernance_factuel)->whereNull('deleted_at')],
