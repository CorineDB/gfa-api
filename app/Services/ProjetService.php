...
class ProjetService extends BaseService implements ProjetServiceInterface
{
        ...
        try {
            if (isset($attributs['organisationId']) && !empty($attributs['organisationId'])) {
                if (!($organisation = app(OrganisationRepository::class)->findById($attributs['organisationId']))) {
                    throw new Exception("Cette organisation n'existe pas", 404);
                }

                if ($organisation->user->programmeId !== Auth::user()->programme->id) {
                    throw new Exception('Cette organisation ne fait pas partir de ce programme', 403);
                }

                if (($projet = $organisation->projet)) {
                    throw new Exception('Cette organisation est déja associé a un projet dans le programme', 403);
                }
            ...
            if (!$owner) {
                throw new Exception('Action impossible a realiser', 403);
            }
            ...
            if ($owner->user->profileable && $owner instanceof Organisation) {
            ...
            if (isset($attributs['sites'])) {
                ...
                foreach ($attributs['sites'] as $id) {
                    if (!($site = app(SiteRepository::class)->findById($id)))
                        throw new Exception('Site introuvable', Response::HTTP_NOT_FOUND);
    ...
    public function update($projetId, array $attributs): JsonResponse
    {
        ...
        try {
            ...
            if (array_key_exists('programmeId', $attributs)) {
                ...
                if ($programme->debut > $attributs['debut'])
                    throw new Exception('La date de début du projet est antérieur à celui du programme', 500);

                if ($programme->fin < $attributs['fin'])
                    throw new Exception('La date de fin du projet est supérieur à celui du programme', 500);
            } else {
                $programme = Auth::user()->programme;
            }
            ...
            if (isset($attributs['organisationId']) && !empty($attributs['organisationId'])) {
                ...
                // Règle 2: Nettoyer les indicateurs de l'ancienne organisation si elle n'a plus de projet
                if ($ancienneOrganisation) {
                    // Vérifier si l'ancienne organisation n'a plus de projet après le changement
                    $ancienneOrganisation->refresh();
                    if (!$ancienneOrganisation->projet) {
                        // Supprimer tous les liens avec les indicateurs (soft delete)
                        DB::table('indicateur_responsables')
                            ->where('responsableable_type', \App\Models\Organisation::class)
                            ->where('responsableable_id', $ancienneOrganisation->id)
                            ->whereNull('deleted_at')
                ...
                if (auth()->user()->profilable_type === UniteeDeGestion::class) {
                    ...
                    if ($ancienneOwner) {
                        // Vérifier si l'ancienne organisation n'a plus de projet après le changement
                        $ancienneOwner->refresh();
                        if (!$ancienneOwner->projet) {
                            // Supprimer tous les liens avec les indicateurs (soft delete)
                            DB::table('indicateur_responsables')
                                ->where('responsableable_type', \App\Models\Organisation::class)
                                ->where('responsableable_id', $ancienneOwner->id)
                                ->whereNull('deleted_at')
                                ->update(['deleted_at' => now()]);
                        }
                    }
    ...
    public function composantes($id = null): JsonResponse
    {
        try {
            ...
            if ($id !== 'undefined' && $id != null)
                $projet = $this->repository->findById($id);  // Retourner les données du premier projet
            else
                $projet = $this->repository->firstItem();  // Retourner les données du premier projet

            if (!$projet)
                throw new Exception("Ce projet n'existe pas", 500);
    ...
    {
        try {
            if (!($projet = $this->repository->findById($id)))
                throw new Exception("Ce projet n'existe pas", 500);
    ...
    public function findById($projetId, array $attribut = ['*'], array $relations = [], array $appends = []): JsonResponse
    {
        try {
            ...
            if (isset($projet)) {
                ...
                throw new Exception("Cet projet n'existe pas", 400);
        } catch (\Throwable $th) {
            return response()->json(['statut' => 'error', 'message' => $th->getMessage(), 'errors' => []], Response::HTTP_INTERNAL_SERVER_ERROR);
        }
    }
