        ...
        try {
            ...
            if ($organisation != null) {
                ...
                if (($soumission = $evaluationDeGouvernance->soumissionDePerception($paricipant_id, $organisation->id)->first())) {
                    ...
                    return response()->json(['statut' => 'success', 'message' => "Quota des soumissions atteints", 'data' => ['terminer' => true, 'data' => [($organisation->sousmissions_enquete_de_perception()->where('evaluationId', $evaluationDeGouvernance->id)->count() == $organisation->pivot->nbreParticipants), $organisation->sousmissions_enquete_de_perception()->where('evaluationId', $evaluationDeGouvernance->id)->count(), $organisation->pivot->nbreParticipants], 'formulaire_de_gouvernance' => null], 'statutCode' => Response::HTTP_OK], Response::HTTP_OK);
