...
class PlanDecaissementService extends BaseService implements PlanDecaissementServiceInterface
{
    ...
    public function create(array $attributs): JsonResponse
    {
        ...
        try {
            ...
            if (!$trimestreValide) {
                throw new Exception("Le trimestre sélectionné ne correspond pas à la période d'exécution de l'activité " . json_encode($durees->pluck('debut')->toArray()), 500);
            }

            // Get registered trimestres for this activity in the selected year
            $trimestres = $this->repository
                ->allFiltredBy([
            ...
            */
            ...
            if (!in_array($attributs['trimestre'], $validTrimestres)) {
                throw new Exception("L'activité ne peut pas être exécutée pour le trimestre {$attributs['trimestre']}", 500);
            }
    ...
    public function update($planDecaissementId, array $attributs): JsonResponse
    {
        ...
        try {
            ...
            if (!($plan = $this->repository->findById($planDecaissementId))) throw new Exception("Cet plan n'existe pas", 500);
            ...
            if (!($plan->trimestre == $attributs['trimestre'])) throw new Exception("Vous pouvez pas modifier le trimestre", 500);
